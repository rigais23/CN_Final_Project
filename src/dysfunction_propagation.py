...
##############################
# SEEDING STRATEGIES (initial infected nodes))
##############################
...
def seeds_disease_genes(G, disease_genes, n_seeds):
    """
    Seed with disease-associated proteins present in the network.
    """
    # disease_genes --> list of gene names associated with the disease (DisGeNET...)
    present = [g for g in disease_genes if g in G]
    # Sort by degree so we seed the most connected disease genes first
    present_sorted = sorted(present, key=lambda n: -G.degree(n))
    seeds = present_sorted[:n_seeds]
    if len(seeds) < n_seeds:
        hubs = seeds_top_hubs(G, n_seeds * 2) # Falls back to top hubs if fewer than n_seeds disease genes are found.
        for h in hubs:
            if h not in seeds:
                seeds.append(h)
                if len(seeds) == n_seeds:
...
def plot_sir(curves, sweep_df, net_name, plot_file):
    ...
    for strategy, grp in curves.groupby('strategy'):
        ...
        ax.plot(grp_sorted['step'], grp_sorted['frac_I'],
                color=colors[strategy], label=labels[strategy], linewidth=2)
    ax.set_title('Infected fraction over time', fontsize=13, fontweight='bold')
    ...
    for strategy, grp in curves.groupby('strategy'):
        grp_sorted = grp.sort_values('step')
        ax.plot(grp_sorted['step'], grp_sorted['frac_R'],
                color=colors[strategy], label=labels[strategy], linewidth=2)
    ax.set_title('Cumulative dysfunction (R)', fontsize=13, fontweight='bold')
    ax.set_xlabel('Timestep')
    ...
    ax = axes[2]
    ax.plot(sweep_df['lambda_'], sweep_df['mean_final_R'],
            color='cornflowerblue', linewidth=2)
    ax.fill_between(sweep_df['lambda_'],
                    sweep_df['mean_final_R'] - sweep_df['std_final_R'],
                    sweep_df['mean_final_R'] + sweep_df['std_final_R'],
                    color='lightblue', alpha=0.4)
    ax.set_title('Epidemic threshold sweep', fontsize=13, fontweight='bold')
    ax.set_xlabel('Spreading probability λ')
    ax.set_ylabel('Final R fraction')

    fig.suptitle(f'SIR DYSFUNCTION PROPAGATION: {net_name.upper()}',
